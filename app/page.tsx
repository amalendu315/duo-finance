 ...
 * Architecture: Modular Sub-Components for stable rendering and focus management.
 * Updates:
 * - Rupee (₹) Symbol integration.
 * - ESLint compliant hydration (no synchronous setState in useEffect).
 * - Full date navigation and history support.
 */
...
const DashboardView: React.FC<DashboardViewProps> = ({
                                                         ...
                                                         filteredTransactions, budgetStats, setView, setShowSettleModal, onDelete
                                                     }) => (
    <div className="pb-24 animate-in fade-in">
        <div className="flex justify-between items-end mb-6">
            <div>
...
const BudgetPlannerView: React.FC<BudgetPlannerViewProps> = ({
                                                                 budgetStats, filter, setFilter, selectedDate, onPrev, onNext, budgets, setBudgets
                                                             }) => {
    return (
        <div className="pb-24 animate-in fade-in">
...
export default function DuoFinancePage() {
    ...
    // --- Persistence Safe for Next.js ---
    useEffect(() => {
        // Avoid synchronous setState in effect by deferring with setTimeout
        const timer = setTimeout(() => {
            const saved = localStorage.getItem(STORAGE_KEY);
            if (saved) {
                try {
